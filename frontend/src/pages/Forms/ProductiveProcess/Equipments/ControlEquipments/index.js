...
export default function ControlEquipments({
  controlEquipments,
  ...
  function handleUpdate(e) {
    const equipments = controlEquipments.map((equipment) =>
      equipment.id === e.id ? e : equipment
    );

    onChangeControlEquipments(equipments);
  }

  function handleDelete(id) {
    onChangeControlEquipments(
      controlEquipments.filter(
        (cotntrolEquipment) => cotntrolEquipment.id !== id
      )
    );
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      onChangeControlEquipments([...controlEquipments, response.data]);
      setIdentification('');
      setAmount('');
      setDate('');
      setFuel('');
  ...
  return (
    <>
      ...
      {controlEquipments.length > 0 &&
        controlEquipments.map((equipment, index) => (
          <Accordion
            key={equipment.id}
            number={index + 1}
